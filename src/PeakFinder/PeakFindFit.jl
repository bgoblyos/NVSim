...
begin
	testData = jldopen("../../data/staticSweepWindow.jld2")
	Bs = testData["Bs"]
	freqs = testData["freqs"]
	signals = testData["signals"]
	md"Data import"
end
...
function simKernel!(scores, Rxs, Rys, Rzs, Bs, peakMatrix)
	...
	if i <= size(scores)[1] && j <= size(scores)[2] && k <= size(scores)[3]
		...
		for x in 1:length(Bs)
			...
			for y in 1:8
				...
				@inbounds scores[i, j, k] += (peakMatrix[x, y] - sortedPeaks[y])^2
			end
		end
	end
    return
...
# ╔═╡ abc18f64-fdcb-49ba-8154-0fd752062a7b
function simKernelAlt!(scores, αs, βs, γs, Bs, peakMatrix)
    i = (blockIdx().x - 1) * blockDim().x + threadIdx().x
	j = (blockIdx().y - 1) * blockDim().y + threadIdx().y
	k = (blockIdx().z - 1) * blockDim().z + threadIdx().z
...
function computeScores(αs, βs, γs)
	n1 = length(αs)
	n2 = length(βs)
	n3 = length(γs)
	...
	@cuda fastmath=true threads=threads blocks=blocks simKernel!(scores, Rxs, Rys, Rzs, cu(Bs), cu(peakMatrix))
...
function fitODMR(αs, βs, γs)
	...
	β = βs[coords[2]]
	γ = γs[coords[3]]
	return (bestfit = bestfit, orientation = (α = α, β = β, γ = γ))
end
...
CUDA.@profile segmentedFit(2)

# ╔═╡ 174177e9-f82d-4cf4-a396-bfea946d13b3
result = segmentedFit(9)
