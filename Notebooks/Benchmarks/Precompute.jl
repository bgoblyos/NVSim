...
# ╔═╡ f6bf21be-a40b-4867-ba5a-7787ad3ec77c
const z = SA_F32[0, 0, 1]

# ╔═╡ 9584d606-6696-434a-afcd-eb7e9bf6b295
const I = SMatrix{3,3,Float32}( 1, 0, 0,
	                            0, 1, 0,
	                            0, 0, 1  )

# ╔═╡ 2647bba2-454f-42e9-839a-413a21c906fa
const n = 512

# ╔═╡ bc31145a-4d0c-41c9-ba8e-10af53d24bf5
md"#### Rotation functions"

# ╔═╡ 53418335-61d8-44f4-a6d5-056181fad6fb
...
# ╔═╡ 103c7a8c-f0a7-4330-9f6c-ab11db0e8bb9
function Ropt(θ, ϕ) # rotation matrix that brings the given polar vector to (0,0,1)
	u = SVector(
		sin(θ) * sin(θ),
		-sin(θ) * cos(ϕ),
		0
...
# ╔═╡ bd427d13-ff1e-4d7a-98c7-43ad2a883fe7
function calculateZeemansOpt(θ, ϕ)
	rot = Ropt(θ, ϕ)
	return SVector(
		unscaledZeeman(rot * dirs[1]),
		unscaledZeeman(rot * dirs[2]),
...
function noPrep(Bmax)
	...
	dαs = CuArray{Float32}(range(0, π/2, n+1)[2:end])
	dβs = CuArray{Float32}(range(0, π/2, n+1)[2:end])
...
@benchmark (CUDA.@sync noPrep(x)) samples=100 seconds = 10 setup=(x = rand((10:500)))

# ╔═╡ b8e05222-a0c2-4872-a007-e0ac336cca0c
md"""
## Precalculated rotation
"""

# ╔═╡ e37e2be0-c3d2-416f-96e7-f11a11aaf918
function getB(θ, ϕ)
	return SVector(
		sin(θ) * cos(ϕ),
		sin(θ) * sin(ϕ),
...
# ╔═╡ 347c799d-9334-442d-adf9-409268fc8e04
function preRot!(scores, Rs, Bs)
    i = (blockIdx().x - 1) * blockDim().x + threadIdx().x
	j = (blockIdx().y - 1) * blockDim().y + threadIdx().y
	k = (blockIdx().z - 1) * blockDim().z + threadIdx().z
	...
	if i <= size(scores)[1] && j <= size(scores)[2] && k <= size(scores)[3]
		...
		@inbounds dir1 = R * dirs[1]
		Hz1 = unscaledZeeman(dir1)
		@inbounds dir2 = R * dirs[2]
		Hz2 = unscaledZeeman(dir2)
		@inbounds dir3 = R * dirs[3]
		Hz3 = unscaledZeeman(dir3)
		@inbounds dir4 = R * dirs[4]
		Hz4 = unscaledZeeman(dir4)
		
		# Direction 1
		@inbounds vals1 = eigvals(Hnv + B * Hz1)/GHzToK

		# Direction 2
		@inbounds vals2 = eigvals(Hnv + B * Hz2)/GHzToK

		# Direction 3
		@inbounds vals3 = eigvals(Hnv + B * Hz3)/GHzToK

		# Direction 4
		@inbounds vals4 = eigvals(Hnv + B * Hz4)/GHzToK
...
# ╔═╡ 8cfa47aa-4f08-4dce-9345-7b516a3af5cc
function preRot(Bmax)	
	dαs = CuArray{Float32}(range(0, π/2, n+1)[2:end])
	dβs = CuArray{Float32}(range(0, π/2, n+1)[2:end])
	...
	@cuda fastmath=true threads=threads blocks=blocks preRot!(scores, Rs, dBs)
	
	return findmin(scores)
end

# ╔═╡ 134c09e8-72f0-4eaf-a6de-923f66de9bc3
CUDA.@time preRot(500)

# ╔═╡ 8dc5ea59-8641-4fa8-92cd-55618deba043
@benchmark (CUDA.@sync preRot(x)) samples=100 seconds = 10 setup=(x = rand((10:500)))

# ╔═╡ c2afdfd0-87d6-4482-8c80-85c9e97a0050
md"""
## Precalculated Zeeman
"""
...
# ╔═╡ 5dd0f8a8-996a-4db0-a5ff-8aee805950c5
@benchmark (CUDA.@sync preZeeman(x)) samples=100 seconds = 10 setup=(x = rand((10:500)))

# ╔═╡ 376e849a-2ddb-4785-adfa-547fdfdf43cf
md"""
## Redundant Zeeman, optimized rotation matrix
"""

# ╔═╡ 91606183-0066-4d19-a9c2-b60f6cb35942
function noPrepOpt!(scores, αs, βs, Bs)
    i = (blockIdx().x - 1) * blockDim().x + threadIdx().x
	j = (blockIdx().y - 1) * blockDim().y + threadIdx().y
	k = (blockIdx().z - 1) * blockDim().z + threadIdx().z
...
# ╔═╡ fc76ae47-f385-4131-b98a-f813955f840f
function noPrepOpt(Bmax)
	...
	@cuda fastmath=true threads=threads blocks=blocks noPrepOpt!(scores, dαs, dβs, dBs)

	return findmin(scores)
end

# ╔═╡ 013b4bb2-c9a7-4e4c-878a-5a219f65c7ad
CUDA.@time noPrepOpt(500)

# ╔═╡ 6ecb8178-dcc8-4e0f-aae2-acdc4e108f7b
@benchmark (CUDA.@sync noPrepOpt(x)) samples=100 seconds = 10 setup=(x = rand((10:500)))

# ╔═╡ 4b4e8ee6-27ea-483f-9eb3-9b67b6d68f5f
md"""
## Precalculated and optimized rotation matrix
"""

# ╔═╡ 3d1601fe-4a09-4a6a-9a20-0c1d174129a1
function preRotOpt!(scores, Rs, Bs)
    i = (blockIdx().x - 1) * blockDim().x + threadIdx().x
	j = (blockIdx().y - 1) * blockDim().y + threadIdx().y
	k = (blockIdx().z - 1) * blockDim().z + threadIdx().z
...
# ╔═╡ db8f4719-2210-4809-a7cb-0977e67960c8
function preRotOpt(Bmax)
	dαs = CuArray{Float32}(range(0, π/2, n+1)[2:end])
	dβs = CuArray{Float32}(range(0, π/2, n+1)[2:end])
	...
	@cuda fastmath=true threads=threads blocks=blocks preRotOpt!(scores, Rs, dBs)
	
	return findmin(scores)
end

# ╔═╡ 0a499eb1-ab55-4770-a6ae-d6b66cc77f3e
CUDA.@time preRotOpt(500)

# ╔═╡ 9f98bcc3-1b4a-46e9-8da5-185dd4cbd67b
@benchmark (CUDA.@sync preRotOpt(x)) samples=100 seconds = 10 setup=(x = rand((10:500)))

# ╔═╡ cd672cba-1e9c-4388-917c-c64a2be63036
md"""
## Precalculated Zeeman, optimized rotation matrix
"""

# ╔═╡ 6bb56b41-e15e-4085-bf5b-dc623a5bb268
function preZeemanOpt!(scores, Hzs, Bs)
    i = (blockIdx().x - 1) * blockDim().x + threadIdx().x
	j = (blockIdx().y - 1) * blockDim().y + threadIdx().y
	k = (blockIdx().z - 1) * blockDim().z + threadIdx().z
...
# ╔═╡ c510c0c9-1c9e-45c6-aee4-aacc0bbffc2e
function preZeemanOpt(Bmax)
	dαs = CuArray{Float32}(range(0, π/2, n+1)[2:end])
	dβs = CuArray{Float32}(range(0, π/2, n+1)[2:end])

	dBs = CuArray{Float32}(range(0, Bmax, n))

	Hzs = calculateZeemansOpt.(dαs, dβs')
	...
	@cuda fastmath=true threads=threads blocks=blocks preZeemanOpt!(scores, Hzs, dBs)

	return findmin(scores)
end

# ╔═╡ caa7e199-04c4-46ac-8521-e1d1b61d53d0
CUDA.@time preZeemanOpt(500)

# ╔═╡ 3da43192-63c8-48a5-aaa9-1f91e0d6c324
@benchmark (CUDA.@sync preZeemanOpt(x)) samples=100 seconds = 10 setup=(x = rand((10:500)))
...
manifest_format = "2.0"
project_hash = "7c5d270e39876d41ad9b8c8da5e06a91891de9cc"
